      ...
      {showModal && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", background: "rgba(0,0,0,0.25)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowModal(false)}>
          <div style={{ background: 'linear-gradient(135deg, #112240 80%, #233554 100%)', borderRadius: 18, minWidth: 280, maxWidth: 340, maxHeight: 400, overflowY: "auto", boxShadow: "0 4px 24px #64FFDA22", padding: 20, border: '2px solid #233554', color: '#E6F1FF' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontWeight: 800, fontSize: "1.18rem", marginBottom: 12, color: "#6366f1", textAlign: "center", letterSpacing: 0.1, textShadow: '0 2px 8px #0A192F44' }}>Yorum Yapanlar</div>
            {loadingCommenters ? (
              <div style={{ textAlign: "center", color: "#b0b3c6", fontSize: 15 }}>Yükleniyor...</div>
            ) : commenters.length === 0 ? (
              <div style={{ textAlign: "center", color: "#b0b3c6", fontSize: 15 }}>Henüz kimse yorum yapmamış.</div>
            ) : (
                  ...
                  <li key={user.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: "1px solid #233554", cursor: "pointer" }}>
                    <Link to={`/profile/${user.id}`} style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: 'inherit', width: '100%' }}>
                      {user.profilePhotoUrl ? (
                        <img src={`http://localhost:5000${user.profilePhotoUrl}`} alt={user.userName} style={{ width: 32, height: 32, borderRadius: "50%", objectFit: "cover", border: "1.5px solid #6366f1", boxShadow: '0 1px 4px #6366f133' }} />
                      ) : (
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#233554", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#6366f1", fontSize: 17, border: '1.5px solid #6366f1' }}>{user.userName?.[0]?.toUpperCase() || "K"}</div>
                      )}
                      <span style={{ fontWeight: 600, color: "#6366f1", fontSize: 15 }}>@{user.userName || user.fullName || "Kullanıcı"}</span>
                    </Link>
                  </li>
                ))}
              </ul>
